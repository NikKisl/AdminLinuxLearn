sudo bash &&
yum install -y rpmdevtools gcc  make wget gd-devel automake  yum-utils perl-devel zlib-devel createrepo pcre-devel GeoIP-devel openssl-devel libxslt-devel openldap-devel perl-ExtUtils-Embed git tree nano &&
...
%if 0%{?rhel} == 7
BuildRequires: redhat-lsb-coreecho "#
%define nginx_home %{_localstatedir}/cache/nginx
%define nginx_user nginx
%define nginx_group nginx
%define nginx_loggroup adm
...
- Initial release" > ~/rpmbuild/SPECS/nginx.spec &&
...
sudo rpm -i ~/rpmbuild/RPMS/x86_64/nginx-1.10.1-1.el7.ngx.x86_64.rpm &&
