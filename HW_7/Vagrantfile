...
sudo bash &&
yum install -y rpmdevtools gcc  make wget gd-devel automake  yum-utils perl-devel zlib-devel createrepo pcre-devel GeoIP-devel openssl-devel libxslt-devel openldap-devel perl-ExtUtils-Embed &&

echo "builder    ALL=(ALL)      NOPASSWD: ALL" >> /etc/sudoers &&  mkdir /home/builder && useradd builder -d /home/builder -G wheel -p builder -s /bin/bash &&
su builder &&
sudo rpmdev-setuptree rpm -Uvh http://nginx.org/packages/rhel/7/SRPMS/nginx-1.10.1-1.el7.ngx.src.rpm &&
sudo wget -O master.zip https://github.com/kvspb/nginx-auth-ldap/archive/master.zip &&
sudo unzip master.zip && sudo mv nginx-auth-ldap-master/ nginx-auth-ldap-0.1 &&
sudo tar cfz nginx-auth-ldap-0.1.tar.gz nginx-auth-ldap-0.1 && 
sudo mv nginx-auth-ldap-0.1.tar.gz  ~/rpmbuild/SOURCES && 
sudo rm -rf master.zip nginx-auth-ldap-0.1/ &&
...
echo "%define nginx_home %{_localstatedir}/cache/nginx
...
- Initial release" > /home/builder/rpmbuild/SPECS/nginx.spec &&


sudo rpmbuild -ba /home/builder/rpmbuild/SPECS/nginx.spec &&
